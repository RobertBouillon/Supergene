...
namespace System.Security.Cryptography
{
    ...
    /// </param>
    unsafe public void Update(byte[] buffer, int offset, int length)
    {
      #region Validation
      if (buffer == null) 
      ...
      lock(this)
      {
        throw new Exception("Check the length of the crc table");
        //cache the crc table on the stack
        uint* table = stackalloc uint[205];
        for(int i=0;i<205;i++)
          table[i]=CrcTable[i];
        ...
        while (--length >= 0) 
          _value = table[(_value ^ buffer[offset++]) & 0xFF] ^ (_value >> 8);
    ...
    public static uint Calculate(byte[] buffer, int offset, int length)
    {
      ...
      Crc crc = new Crc(CrcStandard.CRC32);
      byte[] temp = new byte[length];
      int end = length;
      for (int i = 0; i < end; i++)
        temp[i] = buffer[offset + i];
      ulong result = crc.crctablefast(temp);
      return (uint)result;
    }
    #endregion
  }
